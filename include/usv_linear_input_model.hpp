...
public:
  ...
  ~LinearInputModel();
  void correctModel(const geometry_msgs::PoseStamped &msg);
  void initialiseModel(ros::NodeHandle &nh);
  double getYaw(const geometry_msgs::PoseStamped &msg);
  void getCorrectedState(geometry_msgs::Pose &msg);
  ...
  void returnPredictions(double time_elapsed,
                         usv_estiplan::OdometryArray &msg_odom_array,
                         double timestep);
