...
int main(int argc, char **argv){
	...
	ros::Publisher wave_predictor = estiplan.advertise<std_msgs::Float64>("/wave_prediction", 1000);
	...
	for (size_t i = 0; i < wave_components; i++)
	{
		...
		wave_output += fft_msg.amplitude[i]*sin((fft_msg.frequency[i]*2*M_PI*time_elap)+fft_msg.phase[i]);
	}
